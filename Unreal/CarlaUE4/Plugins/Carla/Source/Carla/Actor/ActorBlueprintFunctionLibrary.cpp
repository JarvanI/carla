    ...
    FActorDefinition &Definition)
{
  ...
  ProjectionModel.Type = EActorAttributeType::Int;
  ProjectionModel.RecommendedValues = { TEXT("4") };
  ProjectionModel.bRestrictToRecommended = false;

  //Fisheye Camera Sample Num
  FActorVariation SSAA;
  SSAA.Id = TEXT("ssaa");
  SSAA.Type = EActorAttributeType::Int;
...
void UActorBlueprintFunctionLibrary::SetCamera(
	const FActorDescription &Description,
	ASceneCaptureSensorMulti *Camera)
{
	CARLA_ABFL_CHECK_ACTOR(Camera);
	Camera->SetImageSize(
		RetrieveActorAttributeToInt("image_size_x", Description.Variations, 800),
...
void UActorBlueprintFunctionLibrary::SetCamera(
	const FActorDescription &Description,
	AFisheyeCamera *Camera)
{
	CARLA_ABFL_CHECK_ACTOR(Camera);
    Camera->SetImageSize(
        RetrieveActorAttributeToInt("image_width", Description.Variations, 1080));
    Camera->SetSSAA(
            RetrieveActorAttributeToInt("ssaa", Description.Variations, 2));
