...
void AFisheyeCamera::ColorToImage(const FString& InImagePath, TArray<FColor> InColor, int32 InWidth, int32 InHeight)
{
    ...
    if (Ex.Equals(TEXT("jpg"), ESearchCase::IgnoreCase) || Ex.Equals(TEXT("jpeg"), ESearchCase::IgnoreCase))
    {
        ...
        if (ImageWrapper->SetRaw(InColor.GetData(), InWidth * InHeight, InWidth, InHeight, ERGBFormat::BGRA, 8))
        {
            ...
            for (int k = 0; k < SampleNum; k++)
            {
                for (int l = 0; l < SampleNum; l++)
                {
                    ...
                    if (IsSampleInCircle(Samplei, Samplej))
                    {
                        ...
                        //float theta = 2 * FMath::Atan(FMath::Tan(thetad) / 2);
                        //等距投影 (equidistance projection)
                        //float theta = FMath::Tan(thetad);
                        //等积投影(equisolid angle projection)
                        //float theta = 2 * FMath::Asin(FMath::Tan(thetad) / 2);
                        //正交投影 (orthogonal projection)
                        float theta = FMath::Asin(FMath::Tan(thetad));
...
)
{
    ...
    int PixelColorExpectedStride = ImageWidth;
...
{
    PixelColorExpectedStride = ImageWidth + 8;
}
